...
export default class PetWalker extends PHService {
    ...
    constructor (event: any, contract: any, user: any, token: string, isHero: boolean) {
        ...
        this.button.click = () => {return this.buttonClick();}
        ...
        this.enums.notification = [
            {
            'title': `Start Walking`,
            'body': `You have a pet walk service soon`,
            },
            {
                'title': `Walk Starting`,
                'body': `The walking started now`,
            },
            {
                'title': `Service finalized`,
                'body': `The walking was finalized now`,
            },
            {
                'title': `Payment billed`,
                'body': `You earned $${this.contract.price} for this walk.`,
            },
        ]
    } 
    printTime = (time: number) => {
        time = Math.floor(time / 1000); // convert ms → seconds
    ...
    buttonText = () => {
        const buttons: any = {
            'requested':  this.hero ? 'Start Walk': 'Starting Soon',
            'started': this.hero ? 'End Walk': 'Walk started',
            'ended': this.hero ? 'Review': 'Review',
            'paid': 'Finalized',
            'finalized': 'Finalized'
        }
    ...
    buttonClick = () => {
        const buttons: any = {
            'requested':  this.hero ? ['confirmToUpdateContract', 'Start Walk?'] : [],
            'started': this.hero ? ['confirmToUpdateContract', 'End Walk?']: [],
            'ended': this.hero ?  ['updateContract'] : ['updateContract'],
            'paid': ['seeDetail'],
            'finalized': []
        }
        return buttons[this.step];
